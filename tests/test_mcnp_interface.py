...
def make_source(source_name: str):
    match source_name:
        case "STEP_jetto":
            jetto_source = TokamakNeutronSource(
            ...
            return jetto_source
        case "eqref_OOB":
            eqdsk_source = TokamakNeutronSource(
...
def test_sdef(source_name: str, tmp_path: Path) -> dict[str, str]:
    ...
    for reaction in source.source_type:
        ...
        if local_file_name.endswith("TT"):
            si, sp, sdef_text = get_next_si_and_sp(sdef_text, 2)
        else:
            ...
            assert 1E-3<=ion_temp_mev<=0.1, "Between 1 to 100 keV of ion-temp expected."
            assert sp.data[-1] == SP_4_SCHEME[reaction], "-1 for DT, -2 for DD."
        assert sp.def_number == 2

        # Test radial distribution
        si, sp, sdef_text = get_next_si_and_sp(sdef_text, 3)
        ...
        for i in ds4.data:
            si, sp, sdef_text = get_next_si_and_sp(sdef_text, i)

        # Test end of file 
        assert scroll_and_get_next_data_line(sdef_text, "SP")[0] == "", "EoF expected"
...
def tokenize(line: str, dtype=float):
    ...
    if "." in data[1]:
        return MCNPDefinitionLine(
            line_type, dist_num, None,
            np.array(data[1:], dtype=dtype),
        )
    return MCNPDefinitionLine(
        line_type, dist_num, data[1],
        np.array(data[2:], dtype=dtype),
    )

def get_next_si_and_sp(text_list: list[str], i:int) -> tuple[MCNPDefinitionLine, MCNPDefinitionLine, list[str]]:
